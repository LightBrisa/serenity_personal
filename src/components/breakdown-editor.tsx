...
import { thesisStorageKey } from './saved-draft';
...
export function BreakdownEditor({ defaultThesis, defaultHorizon }: { defaultThesis: string; defaultHorizon: string }) {
  ...
  const continueToResearch = () => {
    window.localStorage.setItem(thesisStorageKey, thesis.trim());
    window.localStorage.setItem('serenity-personal:nvda:horizon', horizon);
    router.push('/research/nvda');
  };
  ...
  return (
    <div className="rounded-[20px] border border-[#d9ddd5] bg-[#fafaf7] p-5 sm:p-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-[10px] font-bold tracking-[0.11em] text-[#7d857f]">可以直接改</p>
          <h2 className="mt-2 text-lg font-semibold tracking-[-0.02em]">我现在想验证的是</h2>
        </div>
      ...
      <label htmlFor="core-thesis" className="mt-6 block text-xs font-semibold text-[#526057]">这句话的核心判断</label>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-[220px_minmax(0,1fr)]">
        <div>
          <label htmlFor="horizon" className="block text-xs font-semibold text-[#526057]">我想看多长时间</label>
        ...
        <label className="mt-1 flex cursor-pointer items-start gap-3 rounded-xl border border-[#dce0d9] bg-[#f2f5f0] px-4 py-3 sm:mt-6">
          ...
          <span><span className="block text-xs font-semibold text-[#3f4d44]">这样理解没有偏</span><span className="mt-1 block text-[11px] leading-4 text-[#7b847e]">先由你确认，再去查材料。</span></span>
        </label>
      </div>

      <div className="mt-6 flex flex-col gap-3 border-t border-[#e1e4de] pt-5 sm:flex-row sm:items-center sm:justify-between">
        <p className="flex items-center gap-2 text-xs text-[#647069]"><CheckCircle2 size={14} className="text-[#4d765d]" /> 下面先核对 4 个前提、3 个优先问题</p>
        <button type="button" disabled={!confirmed || thesis.trim().length < 30} onClick={continueToResearch} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#173e32] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_6px_16px_rgb(23_62_50/18%)] transition hover:bg-[#214c3e] disabled:cursor-not-allowed disabled:bg-[#b9c0bb] disabled:shadow-none">
          按这些问题去查 <ArrowRight size={15} />
        </button>
      </div>
    </div>
  );

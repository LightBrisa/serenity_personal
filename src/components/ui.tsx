...
export function RelationBadge({ relation }: { relation: EvidenceRelation }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold tracking-[0.08em] ${relationStyles[relation]}`}>
      <Icon size={11} strokeWidth={2.4} aria-hidden="true" /> {relation}
    </span>
  );
}
...
export function StateBadge({ state, large = false }: { state: ThesisState; large?: boolean }) {
  return (
    <span className={`inline-flex items-center gap-2 rounded-full border font-bold tracking-[0.1em] ${stateStyles[state]} ${large ? 'px-3.5 py-2 text-[11px]' : 'px-2.5 py-1 text-[10px]'}`}>
      <span className="h-1.5 w-1.5 rounded-full bg-current" /> {state}
    </span>
  );
}
...
const layerLabels: Record<ResearchLayer, string> = {
  ...
  INFORMATION: 'Information / sentiment',
...
export function Confidence({ value }: { value: EvidenceConfidence }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.07em] text-[#818984]" title={`Confidence: ${value}`}>
      Confidence
...
export function AssumptionStatus({ status }: { status: 'HOLDS' | 'UNDER_PRESSURE' | 'UNKNOWN' | 'BROKEN' }) {
  const config = {
    HOLDS: { label: 'Holds', icon: Check, style: 'text-[#3d6b50] bg-[#e4eee3]' },
    UNDER_PRESSURE: { label: 'Under pressure', icon: ShieldAlert, style: 'text-[#8a642f] bg-[#f5ead5]' },
    UNKNOWN: { label: 'Unknown', icon: CircleHelp, style: 'text-[#6f7772] bg-[#eceee9]' },
    BROKEN: { label: 'Broken', icon: AlertTriangle, style: 'text-[#8e4940] bg-[#f0deda]' },
  }[status];
...
export function FixtureNotice({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`flex items-start gap-2.5 rounded-xl border border-[#e3d7c4] bg-[#f7f0e5] text-[#806541] ${compact ? 'px-3 py-2 text-[11px]' : 'px-4 py-3 text-xs leading-5'}`}>
      ...
      <p><strong>Deterministic demo fixture.</strong> Historical snapshot for product demonstration; not live market data or investment advice.</p>
    </div>
  );
}

export function Unknown({ children }: { children?: React.ReactNode }) {
  return <span className="inline-flex items-center gap-1.5 rounded-md border border-dashed border-[#c5c9c3] bg-[#f0f1ed] px-2 py-1 text-[10px] font-bold tracking-[0.08em] text-[#6f7772]">UNKNOWN{children ? ` · ${children}` : ''}</span>;
}

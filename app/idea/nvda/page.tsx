...
export default function IdeaBreakdownPage() {
  return (
    <div className="mx-auto max-w-[1180px] px-5 pb-16 pt-7 sm:px-8 lg:px-10">
      <JourneyProgress current="Breakdown" />
      <div className="mt-7 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#7c857f]">NVDA · Idea breakdown</p>
          <h1 className="mt-3 max-w-3xl font-serif text-[clamp(2.1rem,4vw,3.35rem)] leading-[1.05] tracking-[-0.035em] text-[#17251f]">Make the claim testable before collecting evidence.</h1>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-[#6f7872]">Serenity has interpreted the pasted idea. Edit the hypothesis or horizon, then confirm what the research should investigate.</p>
        </div>
        <FixtureNotice compact />
      </div>

      <section className="mt-8 rounded-[18px] border border-[#dedfd9] bg-[#eceee9] p-5">
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.11em] text-[#7d857f]"><FileText size={13} /> Original idea</div>
        ...
        <p className="mt-3 text-[11px] text-[#8a918c]">Source: {nvdaIdea.sourceLabel} · No source claims have been verified at this stage.</p>
      </section>
      ...
      <section className="mt-6 rounded-[20px] border border-[#d9ddd5] bg-[#f8f8f4] p-5 sm:p-6">
        <p className="text-[10px] font-bold uppercase tracking-[0.11em] text-[#7d857f]">AI interpretation</p>
        <h2 className="mt-2 text-lg font-semibold">Causal chain</h2>
        <p className="mt-2 text-xs leading-5 text-[#7a837d]">Each link must remain true for the original idea to reach its expected outcome.</p>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        <section className="rounded-[20px] border border-[#d9ddd5] bg-[#fafaf7] p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-[#7d857f]">What must remain true</p><h2 className="mt-2 text-lg font-semibold">Key assumptions</h2></div>
            <span className="rounded-full bg-[#e5ebe3] px-2.5 py-1 text-[10px] font-bold text-[#4b6856]">6 extracted</span>
          </div>
        ...
        <div className="space-y-6">
          <section className="rounded-[20px] border border-[#dfd7cb] bg-[#f8f2e9] p-5">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.11em] text-[#8a6844]"><AlertCircle size={13} /> Important uncertainties</div>
          ...
          <section className="rounded-[20px] border border-[#e1d4cd] bg-[#f6ece6] p-5">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.11em] text-[#8e5e48]"><ShieldAlert size={13} /> Risks to seek</div>
      ...
      <section className="mt-6 rounded-[20px] border border-[#d9ddd5] bg-[#fafaf7] p-5 sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-[#7d857f]">Research plan preview</p><h2 className="mt-2 text-lg font-semibold">Questions Serenity will investigate</h2></div><p className="text-[11px] text-[#8a918c]">Designed to find disconfirming evidence, too</p></div>
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {nvdaResearchQuestions.map((question) => <div key={question.id} className="flex gap-3 rounded-xl border border-[#e0e3dd] bg-white p-4"><CircleHelp size={15} className="mt-0.5 shrink-0 text-[#71867a]" /><div><p className="text-sm font-semibold leading-5 text-[#3c4841]">{question.question}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.08em] text-[#919892]">{question.layer.replace('_', ' ')}</p></div></div>)}
        </div>
      </section>
    </div>
  );
